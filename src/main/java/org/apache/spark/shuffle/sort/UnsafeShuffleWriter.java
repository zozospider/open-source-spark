...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
    ...
    public void write(scala.collection.Iterator<Product2<K, V>> records) throws IOException {
        ...
        // generic throwables.
        // 跟踪成功, 以便我们知道是否遇到异常我们执行此操作, 而不是使用标准的 try/catch/re-throw 来处理通用的 throwables.
        // 跟踪异常
        boolean success = false;
        try {
            while (records.hasNext()) {
                // 将数据插入 ShuffleExternalSorter 进行外部排序
    ...
    // 将数据插入 ShuffleExternalSorter 进行外部排序
    // 该方法会将数据进行序列化, 并且将序列化后的数据通过 insertRecord() 方法插入外部排序器中
    @VisibleForTesting
     ...
     *
     * 合并零个或多个溢出文件, 根据溢出数量和 IO 压缩编解码器选择最快的合并策略.
     *
    ...
    private long[] mergeSpills(SpillInfo[] spills) throws IOException {
        ...
        if (spills.length == 0) {
            ...
            // 如果有多个, 合并输出, 合并的时候有 NIO 和 BIO 两种方式
