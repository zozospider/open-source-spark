 ...
 *
 *
 * YarnAllocator 负责从 YARN ResourceManager 请求容器, 并确定当 YARN 满足这些请求时如何处理容器.
 *
 * 此类使用 YARN 的 AMRMClient API (连接 ApplicationMaster 和 ResourceManager 的客户端). 我们通过三种方式与 AMRMClient 进行交互:
 * * a. 告知我们的资源需求, 这将更新有关所请求容器的本地簿记.
 * * b. 调用 "allocate", 它将我们的本地容器请求与 RM 同步, 并返回 YARN 授予我们的所有容器. 这也起到了心跳的作用.
 * * c. 处理授予我们的容器, 以在其中启动执行程序.
 *
 * 此类的公共方法是线程安全的. 更改状态的所有方法均已同步.
 */
// YARN 资源分配器
private[yarn] class YarnAllocator(
   ...
   *
   *
   * 请求资源, 如果 YARN 满足了我们的所有要求, 那么我们将拥有数量与 maxExecutors 相等的容器.
   *
   * 处理 YARN 可能通过在其中启动执行器来授予我们的任何容器.
   *
   * 这必须同步, 因为在此方法中读取的变量会被其他方法突变.
   */
  // 拿到, 处理, 并运行可分配的容器
  // 最终启动的容器部分运行指令为: /bin/java org.apache.spark.executor.YarnCoarseGrainedExecutorBackend
  def allocateResources(): Unit = synchronized {
    ...
    // requests.
    // 轮询 ResourceManager. 如果没有挂起的容器请求, 这将使您心跳加快.
   ...
   *
   *
   * 通过启动执行程序来处理 ResourceManager 授予的容器.
   *
   * 由于 YARN 分配协议的工作方式, 某些健康的竞争条件可能导致我们不再需要 YARN 授予容器. 在这种情况下, 我们将其释放.
   *
   * 可见用于测试.
   */
  // 处理并运行可分配的容器
  ...
  def handleAllocatedContainers(allocatedContainers: Seq[Container]): Unit = {
    ...
    // 匹配剩余的机架.
    // 由于 YARN 的 RackResolver 吞下了线程中断 (请参阅 SPARK-27094), 这可能导致此代码丢失 ApplicationMaster 的中断, 因此请使用单独的线程来执行操作.
  ...
  private def runAllocatedContainers(containersToUse: ArrayBuffer[Container]): Unit = synchronized {
    ...
    for (container <- containersToUse) {
      ...
      // 比较已启动的 Executors 大小和所需要的 Executors 大小
      if (rpRunningExecs < getOrUpdateTargetNumExecutorsForRPId(rpId)) {

        getOrUpdateNumExecutorsStartingForRPId(rpId).incrementAndGet()
        if (launchContainers) {

          // 从 launcherPool (线程池, 用于启动多个 Executor) 中开启一个线程, 启动容器
