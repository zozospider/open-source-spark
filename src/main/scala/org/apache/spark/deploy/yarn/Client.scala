...
private[spark] class Client(
   ...
   *
   * 将运行我们的 ApplicationMaster 的应用程序提交到 ResourceManager.
   *
   * 稳定的 Yarn API 提供了一种方便的方法 (YarnClient#createApplication), 用于创建应用程序和设置应用程序提交上下文.
   * Alpha API 中不提供此功能.
   */
  ...
  def submitApplication(): ApplicationId = {
    ...
    try {
      ...
      // 从我们的 ResourceManager 获取一个新的应用程序
      // step 2. 通过 YARN 的 ResourceManager 要创建一个应用程序
      ...
      //             非集群模式: /bin/java org.apache.spark.deploy.yarn.ExecutorLauncher
   ...
   *
   * 设置用于提交我们的 ApplicationMaster 的上下文.
   * 这使用了 Yarn alpha API 中不可用的 YarnClientApplication.
   */
   ...
   *
   * 设置一个 ContainerLaunchContext 以启动我们的 ApplicationMaster 容器.
   * 这将设置启动环境, java options 以及用于启动 AM 的命令.
   */
   ...
   *
   * 将应用程序提交到 ResourceManager.
   * 如果将 spark.yarn.submit.waitAppCompletion 设置为 true, 它将保持活动状态, 报告应用程序的状态, 直到由于任何原因退出应用程序为止.
   * 否则, 客户端进程将在提交后退出.
