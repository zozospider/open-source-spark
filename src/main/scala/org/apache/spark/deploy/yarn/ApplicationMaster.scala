...
private[spark] class ApplicationMaster(
  ...
  // YARN ResourceManager Client (连接 YARN ResourceManager 的客户端)
  ...
  // 向 ResourceManager 注册当前的 ApplicationMaster, 申请资源
  ...
  private def runDriver(): Unit = {
    ...
    // 启动用户应用程序:
    // 使用单独线程 (线程名为 "Driver") 启动用户应用程序 (应用程序的 main() 方法中包含初始化 SparkContext)
    userClassThread = startUserApplication()

    // 阻塞等待 SparkContext 初始化完成后再往下执行
    // This a bit hacky, but we need to wait until the spark.driver.port property has
    // been set by the Thread executing the user class.
    // 这有点 hacky, 但是我们需要等到执行用户类的线程设置了 spark.driver.port 属性.
   ...
   *
   * 在单独的线程中启动包含 spark driver 的 user class.
   * 如果主例程干净地退出或对于任何 N 都以 System.exit(N) 退出, 我们都认为它是成功的, 对于所有其他情况, 我们都认为是失败的.
   *
   * 返回已启动的用户线程.
   */
  // 使用单独线程 (线程名为 "Driver") 启动用户应用程序 (应用程序的 main() 方法中包含初始化 SparkContext)
  private def startUserApplication(): Thread = {
    ...
    // 加载传入的 --class org.apache.spark.examples.SparkPi
    // 找到 main() 方法
      ...
      override def run(): Unit = {
        try {
          if (!Modifier.isStatic(mainMethod.getModifiers)) {
            ...
            // 执行静态 main() 方法 (main() 方法中包含初始化 SparkContext)
