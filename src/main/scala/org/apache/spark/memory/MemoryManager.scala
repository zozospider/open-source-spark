 ...
 *
 *
 * 抽象内存管理器, 用于 Execution 和 Storage 之间共享内存.
 *
 * 在这种情况下, Execution 内存是指用于 Shuffles, joins, Sorts 和 Aggregations 中的计算的内存,
 * 而 Storage 内存是指用于在集群中 Caching 和传播内部数据的内存.
 * 每个 JVM 有一个 MemoryManager.
 */
// 其实现为: UnifiedMemoryManager
private[spark] abstract class MemoryManager(
                                             conf: SparkConf,
                                             numCores: Int,
                                             // 堆内 Storage 内存 = 可用内存 * 0.3
                                             onHeapStorageMemory: Long,
                                             // 堆内 Execution 内存 = 可用内存 * 0.3
