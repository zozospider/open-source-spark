 ...
 *
 * :: DeveloperApi ::
 * 保留正在运行的 Spark 实例 (主实例或工作实例) 的所有运行时环境对象, 包括序列化程序, RpcEnv, 块管理器, map 输出跟踪器等.
 * 当前, Spark 代码通过全局变量查找 SparkEnv, 因此所有线程都可以访问相同的 SparkEnv.
 * 可以通过 SparkEnv.get 访问它 (例如, 在创建 SparkContext 之后).
 */
@DeveloperApi
class SparkEnv (
...
object SparkEnv extends Logging {
   ...
   * 为 Driver 或 Executor 创建 SparkEnv 的帮助方法.
   */
  private def create(
                      conf: SparkConf,
                      executorId: String,
