 ...
 *
 *
 * RPC 的通信终端, 它定义给定 message 触发什么功能.
 *
 * 可以保证依次调用 `onStart`, `receive` 和 `onStop`.
 *
 * Endpoint 通信终端的生命周期为: constructor -> onStart -> receive* -> onStop
 *
 * 注意: `receive` 可以被同时调用. 如果您希望 `receive` 是线程安全的, 请使用 ThreadSafeRpcEndpoint
 *
 * 如果从 RpcEndpoint 方法之一抛出任何错误 (除了 `onError` 之外), 都会调用带有原因的 `onError`.
 * 如果 `onError` 抛出错误, RpcEnv 将忽略它.
 */
// RPC 通信终端
