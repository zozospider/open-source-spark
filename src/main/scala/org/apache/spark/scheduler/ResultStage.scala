 ...
 *
 * ResultStages 在 RDD 的某些分区上应用函数以计算操作的结果.
 * ResultStage 对象捕获要执行的函数 `func` (将应用于每个 Partition 分区), 以及一组分区 ID (即 `partitions`).
 * 对于 first() 和 lookup() 之类的操作, 某些 Stages (阶段) 可能未在 RDD 的所有分区上运行.
 */
