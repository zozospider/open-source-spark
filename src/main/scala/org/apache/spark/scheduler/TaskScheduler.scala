 ...
 * DAGScheduler.
 *
 * 低级任务计划程序接口, 当前仅由 org.apache.spark.scheduler.TaskSchedulerImpl 实现.
 *
 * 该接口允许插入不同的任务计划程序. 每个 TaskScheduler 都为单个 SparkContext 计划任务.
 * 这些调度程序从 DAGScheduler 的每个阶段获取提交给他们的任务集, 并负责将任务发送到集群, 运行它们, 重试是否有故障以及减轻散乱.
