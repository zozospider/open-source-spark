 ...
 *
 *
 * 在 TaskSchedulerImpl 中的单个 TaskSet 中安排任务.
 * 此类跟踪每个任务, 如果失败则重试任务 (最多重复次数), 并通过延迟调度处理此 TaskSet 的位置感知调度.
 * 它的主要接口是 resourceOffer, 它询问 TaskSet 是否要在一个节点上运行任务, 以及 handleSuccessfulTask / handleFailedTask, 告诉它其任务之一已更改状态 (例如, 完成 / 失败).
 *
 * 线程: 此类仅设计为通过锁定了 TaskScheduler 的代码 (例如, 其事件处理程序) 进行调用. 不应从其他线程调用它.
 */
