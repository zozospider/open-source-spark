 ...
 * (spark.deploy.*).
 *
 * 等待粗粒度 Executors 连接的调度程序后端.
 *
 * 该后端在 Spark 作业期间保留每个 Executor, 而不是在任务完成时放弃 Executor, 并要求调度程序为每个新任务启动一个新的 Executor.
 * 执行程序可以以多种方式启动, 例如用于粗粒度 Mesos 模式的 Mesos 任务或用于 Spark 的独立部署模式 (spark.deploy.*) 的独立进程.
 */
private[spark]
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint extends IsolatedRpcEndpoint with Logging {
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      // 接收到 CoarseGrainedExecutorBackend 向 Driver 发送的消息: LaunchedExecutor (启动 Executor)
      // 发送逻辑在 CoarseGrainedExecutorBackend.receive() 方法中
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      // 收到 CoarseGrainedExecutorBackend 向 driver 发送的 message: RegisterExecutor (注册 Executor 消息)
      ...
      attributes, resources, resourceProfileId) =>
        if (executorDataMap.contains(executorId)) {
          ...
          // 注意: 一些测试期望在我们将 Executor 放入 map 中后得到回复
