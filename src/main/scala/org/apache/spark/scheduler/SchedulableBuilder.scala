 ...
 *
 * 构建可调度的 tree 的接口
 * buildPools: 建立 tree 节点 (pools)
 * addTaskSetManager: 建立叶子节点 (TaskSetManagers)
 */
// 调度构建器
...
// 实现 SchedulableBuilder (调度构建器) 的 FIFOSchedulableBuilder (先进先出调度构建器)
...
// 实现 SchedulableBuilder (调度构建器) 的 FairSchedulableBuilder (公平调度构建器)
