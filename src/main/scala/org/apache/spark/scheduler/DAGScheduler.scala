...
private[spark] class DAGScheduler(
   ...
   *
   * 如果 shuffleIdToMapStage 中存在一个 ShuffleMapStage, 则获取该 Stage (阶段).
   * 否则, 如果不存在改组 ShuffleMapStage, 则此方法将创建除任何丢失的祖先 ShuffleMapStage 之外的 ShuffleMapStage.
   */
  // 获取或创建 ShuffleMapStage (存在递归)
  private def getOrCreateShuffleMapStage(
    ...
    shuffleIdToMapStage.get(shuffleDep.shuffleId) match {
      ...
      case None =>
        ...
        // 最后, 为给定的 shuffle 依赖关系创建一个 stage (阶段).

        // 创建 ShuffleMapStage (存在递归)
   ...
   * regenerating data.
   *
   * 创建一个 ShuffleMapStage, 它生成给定的 shuffle 依赖项的分区.
   * 如果先前运行的 Stage (阶段) 生成了相同的 shuffle 数据, 则此功能将复制先前 shuffle 中仍然可用的输出位置, 以避免不必要地重新生成数据.
   */
  // 创建 ShuffleMapStage (存在递归)
  def createShuffleMapStage[K, V, C](
    ...
    // 创建 ShuffleMapStage, 属性如下:
    // rdd: 依赖的 RDD (ShuffledRDD 依赖的 RDD)
    // parents: 父 Stages (阶段)
   ...
   * 创建与提供的 jobId 关联的 ResultStage.
   */
  // 创建 ResultStage 及其父 Stages (阶段) (存在递归, 可能有多个 ShuffleMapStage)
  // 所有 Stage (阶段) 数量 = shuffle 依赖的数量 + 1
   ...
   *
   * 获取或创建给定的 shuffle 依赖项的父级列表. 将使用提供的 firstJobId 创建新的 Stages (阶段).
   */
  // 获取父 Stages (阶段) (存在递归)
  private def getOrCreateParentStages(shuffleDeps: HashSet[ShuffleDependency[_, _, _]],
                                      firstJobId: Int): List[Stage] = {
    // 将 Shuffle 依赖转换成 ShuffleMapStage
   ...
   *
   *
   * 返回此 Stage (阶段) 的随机依赖关系, 它们是给定 RDD 和与 RDD 关联的 ResourceProfiles 的直接父级.
   *
   * 此函数将不返回更多的远距离祖先以进行随机排序依赖性. 例如, 如果 C 对 B 具有 shuffle 依赖性, 而 B 对 A 具有 shuffle 依赖性:
   * A <-- B <-- C
   * 用 rdd C 调用此函数只会返回 B <-- C 依赖项.
   *
   * 为了进行单元测试, 此功能对于调度程序是可见的.
   */
  // 获取 Shuffle 依赖
  ...
  def submitJob[T, U](
    ...
    // main logic
    // 将 JobSubmitted (event 事件) 放入 eventQueue (事件队列). eventThread (事件线程) 稍后将对其进行处理.
    // eventThread (事件线程) 处理逻辑: 不断从 eventQueue (事件队列) 中获取 event (事件), 并调用 onReceive() 方法进行处理
   ...
   * they arrive.
   *
   * 在给定的 RDD 上运行操作 Job, 并将所有结果在到达时传递给 resultHandler 函数.
   *
   * @param rdd target RDD to run tasks on
   * @param func a function to run on each partition of the RDD
   * @param partitions set of partitions to run on; some jobs may not want to compute on all
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      // 例如, 如果 Job 在其基础 HDFS 文件已被删除的 HadoopRDD 上运行, 则创建新 stage (阶段) 可能会引发异常.
    ...
    // 提交 Stage (阶段) 及其上一级的 Stage (存在递归)
    // 最终的结果就是: 从第一个 Stage (阶段) 开始, 从前往后往后依次提交
  ...
  /** 提交 stage (阶段), 但首先递归提交所有丢失的 parents. */
  // 提交 Stage (阶段) 及其上一级的 Stage (存在递归)
  // 最终的结果就是: 从第一个 Stage (阶段) 开始, 从前往后往后依次提交
  private def submitStage(stage: Stage): Unit = {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        // 从传入的 Stage (阶段) 开始, 从后往前找有没有上一级:
        // 如果有上一级, 则提交上一级 Stage (阶段) (递归调用)
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
   ...
   * DAG scheduler 的主事件循环.
   */
  // 实现 EventLoop 的 onReceive() 方法
