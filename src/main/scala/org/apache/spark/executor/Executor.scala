 ...
 *
 *
 * Spark Executor, 由 threadpool (线程池) 支持以运行 Tasks (任务).
 *
 * 可以与 Mesos, YARN 和独立调度程序一起使用.
 * 内部 RPC 接口用于与 Driver 进行通信, 但在 Mesos 细粒度模式下除外.
 */
private[spark] class Executor(
  ...
  // 启动工作线程池使用 UninterruptibleThread 运行任务, 这样我们就可以允许正在运行的代码而不会被 `Thread.interrupt()` 中断.
  // 如果某些方法被中断, 则某些问题 (例如KAFKA-1894, HADOOP-10622) 将永远挂起.
  ...
  def launchTask(context: ExecutorBackend, taskDescription: TaskDescription): Unit = {
    ...
    // 创建 TaskRunner (任务可运行线程对象)
    val tr = new TaskRunner(context, taskDescription, plugins)
    runningTasks.put(taskDescription.taskId, tr)

    // 在 threadPool (ThreadPoolExecutor 线程池) 中执行 TaskRunner 线程
